...
std::string MyBigNumber::sum(std::string stn1, std::string stn2) {
    ...
    // Check to erase leading '0's in stn1 and stn2 strings
    while (stn1.length() > 1 && stn1[0] == '0') stn1.erase(0, 1);
    while (stn2.length() > 1 && stn2[0] == '0') stn2.erase(0, 1);

    // Special case: empty string
    if (stn1.empty() && stn2.empty()) return "";
    else if (stn1.empty()) return stn2;
    else if (stn2.empty()) return stn1;
